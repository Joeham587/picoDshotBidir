...
private:
  ...
  static constexpr const uint8_t gcr_raw_bitlengths[] = {
      ...
      /*11 consecutive bits */ 4, // Not valid, but sometimes occurs at the end
                                  // of the string
      ...
      // more than 10 consecutive samples, means 4 0 or 1 in a row is invalid in
      // GCR
  };
...
public:
  ...
  bool decodeTelemetry(const uint64_t &raw_telemetry,
                       DShot::Telemetry &telemetry) {
    uint16_t result = 0;
    uint32_t gcr_result = 0;
    telemetry.reads++;

    // telemetry data must start with a 0, so if the first bit is high, we don't
    // have any data
    ...
    for (uint64_t mask = 0x4000000000000000; mask; mask = (mask >> 1)) {
      if (((raw_telemetry & mask) != 0) !=
          current) { // if the doesn't match the current string of bits, end the
                     // current string and flip current
        ...
        if (current)
          gcr_result |= gcr_raw_set_bits[gcr_raw_bitlengths[consecutive]];
        // count bits_found (for debugging)
        bits_found += gcr_raw_bitlengths[consecutive];
        // invert current, and reset consecutive
        current = !current;
        consecutive =
            1; // first bit found in the string is the one we just processed
      } else { // otherwise incriment consecutive count
        consecutive++;
        if (consecutive > 16) {
          printf(" Invalid consecutive count of 16\n");
          // invalid run length at the current sample rate (outside of
          // gcr_raw_bitlengths table)
          telemetry.errors++;
          return false;
        }
      }
    ...
    // outside the loop, we still need to account for the final bits if the
    // string ends with 1s bitshift gcr_result by N, and
    gcr_result = (gcr_result << gcr_raw_bitlengths[consecutive]);
    // then set set N bits in gcr_result, if current is 1
    if (current)
      gcr_result |= gcr_raw_set_bits[gcr_raw_bitlengths[consecutive]];
    ...
    // break the 20 bit gcr result into 4 quintets for converting back to
    // regular 16 bit binary
    ...
    if (result_crc != calc_crc) {
      printf(" Expected CRC: 0x%x ", result_crc);
      printf(" but computed 0x%x\n", calc_crc);
      telemetry.errors++;
      return false;
    }

    // todo: properly handle zero eRPM value (min representable eRPM period)
    if (result & 0x100) { // eRPM value, not extended telemetry. todo: reduce
                          // magic numbers here
      telemetry.eRPM_period_us =
          (uint32_t)(result) &
          (uint32_t)0b111111111; // period base is 9 bits above 4 bit CRC
      telemetry.eRPM_period_us =
          telemetry.eRPM_period_us
          << (result >>
              9); // period base is the left shift amount stored in the 3 MSB
      // Serial.print(" period: ");
      // Serial.println(telemetry.eRPM_period_us);
      telemetry.rpm =
          ONE_MINUTE_OF_MICROSECONDS / (telemetry.eRPM_period_us * poles /
                                        2); // /2 due to pole pairs, not poles
                                            // Serial.print(" rpm: ");
      ...
      case DSHOT_EXT_TELEMETRY_TEMPERATURE:
        // Serial.print(" T.Temp: "); Serial.print(telemetry_value);
        // Serial.println("C");
        telemetry.temperature_C = telemetry_value;
        break;
      case DSHOT_EXT_TELEMETRY_VOLTAGE:
        telemetry.volts_cV =
            (uint16_t)telemetry_value * 25; // comes in as 0.25V/bit
        // Serial.print(" T.Voltage: ");
        // Serial.print((float)telemetry_value/4.0f, 2); Serial.println("V");
        break;
      case DSHOT_EXT_TELEMETRY_CURRENT:
        telemetry.amps_A = telemetry_value;
        // Serial.print(" T.Current: "); Serial.print(telemetry_value);
        // Serial.println("A");
        break;
      default:
        // Serial.print(" T.Other: "); Serial.print(telemetry_type, HEX);
        // Serial.print(" val: "); Serial.println(telemetry_value, HEX);
        break;
      }
    }
